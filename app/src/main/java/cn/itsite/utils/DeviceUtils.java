...
public class DeviceUtils {
	...
	public static int getDefaultLoadFactor() {
		if (_loadFactor == null) {
			Integer integer = Integer.valueOf(0xf & BaseApplication.getContext().getResources().getConfiguration().screenLayout);
			_loadFactor = integer;
	...
	public static int getStatusBarHeight() {
		...
		try {
			...
			return BaseApplication.getContext().getResources().getDimensionPixelSize(x);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
	...
	public static boolean isPackageExist(String pckName) {
		try {
			PackageInfo pckInfo = BaseApplication.getContext().getPackageManager().getPackageInfo(pckName, 0);
			if (pckInfo != null)
				return true;
	...
	public static void hideSoftKeyboard(View view) {
		if (view == null)
			return;
		((InputMethodManager) BaseApplication.getContext().getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(view.getWindowToken(), 0);
	}
	...
	public static String getCurCountryLan() {
		return BaseApplication.getContext().getResources().getConfiguration().locale.getLanguage() + "-" + BaseApplication.getContext().getResources().getConfiguration().locale.getCountry();
	}

	public static boolean isZhCN() {
		String lang = BaseApplication.getContext().getResources().getConfiguration().locale.getCountry();
	...
	public static PackageInfo getPackageInfo(String pckName) {
		try {
			return BaseApplication.getContext().getPackageManager().getPackageInfo(pckName, 0);
		} catch (NameNotFoundException e) {
			e.printStackTrace();
		}
		return null;
